...
std::string Sha256::hexDigest(const std::string &data) {
    ...
    // Pre-processing (padding): append a single '1' bit, then '0' bits,
    // then the original message length as a 64-bit big-endian integer,
    // so the total length is a multiple of 512 bits (64 bytes).
    ...
    for (std::size_t chunkStart = 0; chunkStart < message.size(); chunkStart += 64) {
        ...
        for (int i = 0; i < 64; ++i) {
            ...
            const std::uint32_t temp1 = hh + s1 + ch +
                                        kRoundConstants[static_cast<std::size_t>(i)] +
                                        w[static_cast<std::size_t>(i)];
    ...
    for (std::uint32_t word : h) {
        for (int shift = 28; shift >= 0; shift -= 4) {
            out << "0123456789abcdef"[(word >> shift) & 0xf];
        }
    }
    return out.str();
}
