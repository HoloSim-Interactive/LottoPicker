...
| DATA-IN-101 | Each ingested draw record is tagged with the number-pool size in effect on its draw date, derived from a documented table of Florida Lotto rule-change dates, so downstream scoring can normalize eras instead of assuming a constant pool size across all history. Verified on part 1 (synthetic-fixture era-tagging mechanism) since issue #10; part 2 (real-CSV boundary cross-check) closed on issue #27 once real client data arrived — boundary corrected to 1999-10-24 (was 1999-01-01) — see TP-DATA-IN-101 note. | SN-6 | Test | Verified (TP-DATA-IN-101 part 1 passed by Test Engineer, issue #10, 13/13; merged to main by CI/CD, HEAD 4a56564; trunk regression passed by Test Engineer, 35/35, no regressions from concurrent issue-9/issue-10 merge, HEAD c44ed31. Part 2 real-CSV boundary correction to 1999-10-24 passed by Test Engineer, issue #27, full suite 88/88, `[DATA-IN-101]` filter 8/8, independently re-derived from raw CSV; verified `issue-27` @ `d6acb0d`; merged to main by CI/CD, HEAD `e578223`, tag v1.0.249, no release cut, 88/88 post-merge; trunk regression confirmed clean by Test Engineer at `main` @ `b4a8adb`, full suite 88/88, no concurrent changes, clean tree, `clang-format` clean aside from pre-existing `Version.h` nit) | 4a56564, e578223 (tag v1.0.249) |
...
| CORE-202 | Composite ranking metric: combine the per-number decay score (CORE-200) and all applicable co-occurrence group scores (CORE-201) into one score per full 6-number combination, per the weighting approach selected and documented via the CORE-207 research task. | SN-1, SN-5 | Test | Verified (TP-CORE-202 passed by Test Engineer, issue #15, 47/47 incl. 5 new `CompositeScorerTest.cpp`; merged to main by CI/CD, tag v1.0.138, no release cut; trunk regression confirmed clean 47/47 at 5b1a954, no concurrent changes) | 6117db2 (tag v1.0.138) |
| CORE-203 | Score the full ~22.9M-combination space and retain only the configured top-N (default 100) without materializing the full ranked list in memory. | SN-1 | Test + Analysis | Verified (TP-CORE-203 both parts passed by Test Engineer, issue #17, 74/74 incl. new `RankingEngineTest.cpp`; manual end-to-end run against a 5-draw fixture confirmed sane top-10 output, ~55-56s wall-clock for the full 22.9M-combination scan, consistent with the documented `O(C(n,6) log top_n)` complexity, not a hang; trunk regression confirmed clean post-merge on `main` @ f04b68a, 74/74, no concurrent changes) | 890ce24 (tag v1.0.161) |
| CORE-204 | Persist the derived statistical model (per-number decay + per-group co-occurrence scores) as its own data artifact, computed once and reused across ranking runs unless the underlying historical data has changed. | SN-1 | Test | Verified (TP-CORE-204 passed by Test Engineer, issue #16, 59/59; merged to main by CI/CD, tag v1.0.145, no release cut, 64/64 post-merge; trunk regression confirmed clean at main@2d40281, 64/64, both TP-CORE-204 parts re-verified on trunk binary, no concurrent changes) | 86ca3ed (tag v1.0.145) |
| CORE-205 | Backtest engine: for a chosen sample date, run the model using only data available up to that date, then report the actual draw's rank/percentile (or "not found in top-N") and partial-match containment counts at the 3/4/5/6 level. | SN-2, SN-3 | Test | Verified (TP-CORE-205 both parts passed by Test Engineer, issue #20, 80/80 incl. 5 new `BacktestEngineTest.cpp` cases — hand-verified containment counts 20/45/18/1 at levels 3/4/5/6 for the found case, `C(9,6)=84` fixture; not-found case and both `BacktestError` cases also verified; manual end-to-end `--backtest` run against a 6-draw fixture confirmed real wiring, ~55s at pool 53; clang-format re-verified clean after a formatting-only fixup, 9589e10; percentile formula (within retained top-N envelope) and chance-expected-containment reuse of CORE-206's hypergeometric baseline documented as judgment calls in code comments, not yet exercised by a dedicated assertion; merged to trunk e9d03d8/v1.0.194 by CI/CD, one additional pre-merge clang-format fix outside TE's recheck scope (`ModelStore.cpp:161`), one concurrent-race conflict against issue #21 resolved mechanically, full suite 83/83 clean, final HEAD a223539; trunk regression confirmed clean by Test Engineer at main @ 3ab34fa, 142/142 build targets, 83/83 tests, clang-format clean across src/include/tests except pre-existing Version.h nit, CLI `--backtest` usage line confirmed, no regressions) | e9d03d8, a223539 (tag v1.0.194) |
| CORE-206 | Historical pool-size normalization: draws recorded under a different historical pool size are incorporated into the current-era model via a documented, justified normalization method, not discarded. The requirement (normalize, don't discard) is confirmed; normalization method resolved in `docs/SDD.md` §Architecture → Algorithm Design (observed-minus-chance-expected via each era's hypergeometric baseline), informed by CORE-207's research. | SN-6 | Test | Verified (TP-CORE-206 all 3 parts passed by Test Engineer, issue #14, 42/42 incl. 8 new `PoolSizeNormalizerTest.cpp`; merged to main by CI/CD; trunk regression confirmed clean, 42/42, no concurrent changes) | d235ced |
...
| DATA-OUT-300 | Ranked-list output structure: ordered (rank, 6-number combination, composite score) records for the retained top-N. | SN-1 | Test | Verified (TP-DATA-OUT-300 passed by Test Engineer, issue #19; new `[DATA-OUT-300][CORE-203]`-tagged test added to `tests/RankingEngineTest.cpp` on `issue-19` exercising `top_n=5`, exactly 5 entries, strictly descending scores, each combination 6 distinct numbers in valid pool range; 75/75 full suite; merged to main by CI/CD; trunk regression confirmed clean by Test Engineer at main @ ddcd2fa, 75/75, no concurrent changes) | 3d23a0c |
| DATA-OUT-301 | Persisted statistical model schema (pairs with CORE-204): documented file format holding per-number decay scores, per-group co-occurrence scores, and metadata (source data identity/hash, date range covered) sufficient to detect staleness. | SN-1 | Test | Verified (implementation already delivered in issue #16/CORE-204, `ModelSerializer.h/.cpp`; TP-DATA-OUT-301 formally exercised and passed by Test Engineer, issue #18: `[DATA-OUT-301]` filter 8/8 assertions, 6/6 cases incl. non-short-decimal-representable fixture values confirming bit-for-bit round-trip; full suite 64/64; clang-format clean; no code diff vs main, so no new commit) | 86ca3ed (tag v1.0.145, from issue #16) |
| DATA-OUT-302 | Backtest report structure (pairs with CORE-205): per sampled year, the actual draw's numbers, its rank/percentile (or "not found"), and partial-match containment counts for 3/4/5/6. | SN-2, SN-3 | Test | Verified (TP-DATA-OUT-302 passed by Test Engineer, issue #22; `BacktestResult` (from CORE-205/issue #20) confirmed as DATA-OUT-302's own report-row type, `std::vector<BacktestResult>` as the full report — same relationship DATA-OUT-300 has to `RankedCombo`; new `[DATA-OUT-302]`-tagged test in `BacktestEngineTest.cpp`, 3-sampled-year fixture (2017/2018/2019), exactly 3 rows all fields populated, containment breakdown 20/45/18/1 at levels 3/4/5/6 hand-verified per year; full suite 84/84, 721 assertions, clang-format clean; real end-to-end `--backtest d1,d2,d3` run against an 8-draw CSV confirmed exactly 3 printed rows with all fields populated; trunk regression confirmed clean by Test Engineer at main @ b0765e1, 84/84, unchanged count, no concurrent changes, `[DATA-OUT-302][CORE-205]` fixture re-run 85/85 assertions, clang-format clean except pre-existing Version.h nit, real end-to-end `--backtest` re-confirmed) | df2337c (tag v1.0.204) |
| OUT-400 | Console presentation of the top-N ranked list (rank, combination, score), human-readable. Console is the only required output destination for MVP — file export was not requested and is not required. | SN-1 | Test | Verified (TP-OUT-400 passed by Test Engineer, issue #21, 78/78 incl. 3 new `[OUT-400]` cases in `RankedListPresenterTest.cpp` — 5-row-in-order fixture, consistent `\|`-delimiter column count, empty-list header-only table; real CLI end-to-end run against a synthetic 3-draw CSV confirmed aligned, delimited output; `clang-format` clean; no regressions in the other 75 tests; merged to main by CI/CD at `6cd0bfc`, tagged v1.0.181, no release cut; trunk regression confirmed clean by Test Engineer on `main` @ `4438ef4` — 78/78 incl. 3 `[OUT-400]` cases, `clang-format` clean, real CLI end-to-end re-confirmed, no concurrent changes) | 6cd0bfc |
| OUT-401 | Console presentation of the backtest report, one row per sampled year, rendering "not found in top-N" distinctly from a numeric rank. | SN-2, SN-3 | Test | Verified (TP-OUT-401 passed by Test Engineer, issue #23, 88/88 incl. 4 new `[OUT-401]` cases in `BacktestReportPresenterTest.cpp` — 3-year fixture (2 found at different ranks + 1 not-found), header+3-row exact content check, "not found in top-N" rendered distinctly from numeric rank with no `-1`/`0` placeholder, consistent `\|`-delimiter columns, empty-report header-only case; `clang-format` clean; real end-to-end `--backtest` run against a synthetic 5-draw fixture confirmed 2 printed report rows (aligned columns, literal "not found in top-N" text) with the errored requested date excluded rather than faked as a row; no regressions in the other 84 tests; trunk regression confirmed clean by Test Engineer on `main` @ `46342d6`, 88/88 passing, 776 assertions, matching CI/CD's merge report exactly, `clang-format` clean, no concurrent changes; CI/CD confirmed `issue-23` is an ancestor of `main` with no diff, so no re-merge/re-tag was produced by the regression pass) | `8ed9543` (merged issue #23, tagged v1.0.213) |
| NFR-500 | The tool operates entirely offline against locally supplied data (client-supplied CSV, local config, local persisted model artifact) — no network calls during a normal ranking or backtest run. | SN-4 | Inspection | Verified (TP-NFR-500 passed by Software Engineer and independently by Test Engineer, issue #24, inspection-only — no code change, full regression 88/88 sanity-checked; CI/CD confirmed no diff pending on `main`) | f217b45 |
| DELIV-900 | Minimal run documentation: build-from-clone steps, the config file format with one complete working example (pointing at a small sample CSV and a top-N value), the exact launch command, and what correct console output looks like. Previously also tracked the deferred DATA-IN-101 part 2 pre-delivery check (confirm/correct the 1988/1999 era boundary against the client's real historical CSV once supplied) — that check closed on issue #27; see TP-DATA-IN-101. | Deliverable Requirements | Inspection + Demonstration | Verified (TP-DELIV-900 passed by Test Engineer, issue #25, clean-clone `cmake --preset default && cmake --build --preset default && ctest --preset default` → 88/88; both documented commands — plain ranking run and `--backtest 2024-03-13` — reproduced README's pasted Sample output byte-for-byte against the new `examples/sample_history.csv`/`sample_config.cfg` fixtures; no undocumented steps; `examples/sample_history.csv.model` correctly `.gitignore`d. Deferred DATA-IN-101 part 2 pre-delivery check closed on issue #27 — real client CSV arrived, boundary confirmed/corrected to 1999-10-24.) | `7fcd590` (merged issue #25, tagged v1.0.231) |
...
2. Real-data check (deferred, not a Verified gate — see status note): era boundary is 1988 (6/49 launch) → 1999 (6/53) per `docs/SDD.md`'s Algorithm Design section — flagged there as a working hypothesis from a secondary source, not the Florida Lottery's own archive. Expected, once the client's real CSV exists: cross-check this boundary against the actual client-supplied CSV's observed number ranges per date range, and correct the one-line era table in code (`EraTable.h::eraTable()`) if the real data disagrees.
   ...
   **Resolved (2026-09-05, issue #27):** the client's real historical data landed (`data/florida_lotto_full.csv`, 4018 records, commit `be8ef7e`) and disagreed with the working-hypothesis boundary. Confirmed boundary is **1999-10-24**, not 1999-01-01 as `EraTable.h` originally held — every draw 1988-05-07…1999-10-23 contains only 1–49, every draw from 1999-10-27 onward contains 1–53 (earliest >49 value is 1999-10-27; the Florida Lottery's documented matrix-change date 1999-10-24 sits in the resulting `(1999-10-23, 1999-10-27]` window; corroborated by 1999-10-27 also being the first Wednesday draw, when the schedule went from weekly-Saturday to twice-weekly). Software Engineer corrected `EraTable.h::eraTable()`'s second boundary to `{"1999-10-24", 53}` and updated the one real-default-table test fixture in `EraTaggerTest.cpp` accordingly (all other fixtures are synthetic and unaffected). Test Engineer independently re-derived the same boundary directly from the raw CSV (own `awk` pass, not just trusting the reported numbers) and confirmed a match. **Part 2 is now closed, not merely deferred** — this was the last item DELIV-900 was tracking pre-delivery. Two related open design questions (Double Play draws putting two draws on one date in `_full.csv`; `X2`-`X5` XTRA-multiplier columns aren't drawn numbers) are noted but out of scope for this correction — flagged for Solutions Architect if/when real-CSV wiring into a run is scoped.
