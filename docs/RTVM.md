...
| CORE-204 | Persist the derived statistical model (per-number decay + per-group co-occurrence scores) as its own data artifact, computed once and reused across ranking runs unless the underlying historical data has changed. | SN-1 | Test | In Test (TP-CORE-204 passed by Test Engineer, issue #16, 59/59; pending CI/CD merge) | |
