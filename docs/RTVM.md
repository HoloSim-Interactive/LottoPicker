...
| NFR-500 | The tool operates entirely offline against locally supplied data (client-supplied CSV, local config, local persisted model artifact) — no network calls during a normal ranking or backtest run. | SN-4 | Inspection | Verified | Inspection-only (no code change); verified against `main` per issue #24 |
