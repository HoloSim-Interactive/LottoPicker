...
| UI-002 | Config file is parsed into, at minimum: (a) path to the historical draw CSV, (b) the top-N envelope size. Either key missing, or top-N not a positive integer, is a validation error naming the offending key. | SN-1, SN-4 | Test | In Test (TP-UI-002 passed by Test Engineer, issue #9, branch `issue-9` @ 5614d2b; pending CI/CD merge) | |
