...
| CORE-202 | Composite ranking metric: combine the per-number decay score (CORE-200) and all applicable co-occurrence group scores (CORE-201) into one score per full 6-number combination, per the weighting approach selected and documented via the CORE-207 research task. | SN-1, SN-5 | Test | In Test (TP-CORE-202 passed by Test Engineer, issue #15, 47/47 incl. 5 new `CompositeScorerTest.cpp`; pending CI/CD merge) | |
